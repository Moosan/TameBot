...
- **スプシ連携**: `SPREADSHEET_API_URL` 設定時、トリガー実行ごとに **5ロール（イケケモ・ケモ案内・[ケモcafe]・ケモ裏方・ケモ情報部）に属するメンバー** の一覧（名前・リアクション・ロール）と集計結果を App Script 経由でスプシに送信。 [appscript/README](./appscript/README.md) 参照。
